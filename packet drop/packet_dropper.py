...
def parse_arguments():
    ...
    parser.add_argument('--drop-rate', type=float, default=0.10, help="Packet drop probability (0 to 1)")
    parser.add_argument('--output-file', type=str, default='received_dropped_0.10.mpg', help="Output video file")
...
def check_prerequisites(input_file, listen_port, forward_port):
    try:
        ...
        logging.error("FFmpeg not found. Ensure it is installed and in PATH.")
        return False
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        try:
            ...
            logging.error(f"Port {listen_port} is in use.")
            return False
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        try:
            s.bind(("0.0.0.0", forward_port))
        except socket.error:
            logging.error(f"Port {forward_port} is in use.")
            return False
    return True
        ...
        while not stop_event.is_set():
            try:
                ...
                if random.random() > drop_rate:
                    ...
                    logging.debug(f"Packet #{packet_count} forwarded")
                else:
                    dropped_count += 1
                    logging.debug(f"Packet #{packet_count} dropped ({dropped_count} total)")
            except socket.timeout:
                continue
    except Exception as e:
        logging.error(f"Packet dropper error: {e}")
    finally:
        listen_sock.close()
        forward_sock.close()
...
def main():
    ...
    try:
        ...
        if os.path.isfile(args.output_file) and os.path.getsize(args.output_file) > 0:
            logging.info(f"Output file saved: {args.output_file}")
        else:
            logging.error(f"No output file created or file is empty: {args.output_file}")
